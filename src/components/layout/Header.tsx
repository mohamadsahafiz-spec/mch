...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={`px-6 py-3.5 border-b sticky top-0 z-20 backdrop-blur-md transition-colors duration-200 flex flex-col md:flex-row md:items-center justify-between gap-4 ${
      isDark 
        ? 'bg-[#111315]/90 border-[#2B323A] text-slate-100' 
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border uppercase tracking-wider flex items-center gap-1 ${
            isDark 
              ? 'bg-[#8B9DFF]/15 text-[#8B9DFF] border-[#8B9DFF]/30' 
              : 'bg-indigo-50 text-indigo-700 border-indigo-200'
          }`}>
            <Sparkles className="w-3 h-3 text-[#8B9DFF]" />
            DIRECTIVE
          </span>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            {alerts.length > 0 && (
              <span className={`absolute -top-1 -right-1 w-4 h-4 rounded-full text-[10px] font-bold font-mono flex items-center justify-center text-white ${criticalAlertsCount > 0 ? 'bg-[#E98A8A] animate-pulse' : 'bg-[#EFCB7A]'}`}>
                {alerts.length}
              </span>
            )}
          </button>
          ...
          {showAlertsDropdown && (
            <div className={`absolute right-0 mt-2 w-80 border rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 ${
